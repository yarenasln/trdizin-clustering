...
# d3 / d2 yeterince küçükse üçüncü etiketi de ekle
SECOND_RATIOS = np.arange(1.05, 1.26, 0.01)

THIRD_RATIOS = np.arange(1.02, 1.16, 0.01)
...
print("=" * 110)
print("TUNED ADAPTIVE MULTI-LABEL SEEDED K-MEANS")
print("=" * 110)
...
validation_true_average = float(
    y_validation_true.sum(axis=1).mean()
)

TARGET_AVERAGE_LABELS = 2.50
LABEL_COUNT_PENALTY = 0.01

search_df["Target_Label_Error"] = (
    search_df["Average_Predicted_Labels"]
    - TARGET_AVERAGE_LABELS
).abs()

search_df["Tuned_Score"] = (
    search_df["Micro_F1"]
    - LABEL_COUNT_PENALTY
    * search_df["Target_Label_Error"]
)
...
print(
    ...
    round(validation_true_average, 2)
)

print(
    "Hedef ortalama etiket:",
    TARGET_AVERAGE_LABELS
)

print(
    "Tuned score:",
    round(float(best_row["Tuned_Score"]), 4)
)
...
print()
print("=" * 110)
print("FINAL TUNED ADAPTIVE MULTI-LABEL K-MEANS TEST")
print("=" * 110)
...
search_df.to_csv(
    os.path.join(
        OUTPUT_DIR,
        "tuned_adaptive_kmeans_rule_search.csv"
    ),
    index=False,
    encoding="utf-8-sig"
)
...
prediction_df.to_csv(
    os.path.join(
        OUTPUT_DIR,
        "tuned_adaptive_kmeans_predictions.csv"
    ),
    index=False,
    encoding="utf-8-sig"
)
...
summary = pd.DataFrame(
    [
        {
            "Method":
                "Tuned Adaptive Multi-label Seeded K-Means",
...
summary.to_csv(
    os.path.join(
        OUTPUT_DIR,
        "tuned_adaptive_kmeans_summary.csv"
    ),
    index=False,
    encoding="utf-8-sig"
)
